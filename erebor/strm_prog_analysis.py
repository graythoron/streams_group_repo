...
def density_median(sample: np.ndarray, circular: bool = False):
    """
    Calculates the median using a KDE.
    Parameters
    ----------
    sample : np.array-like
        The sample to find the median of.
    circular : boolean
...
def to_skycoords(
    xyz: np.ndarray | List, vels: np.ndarray | None = None, coordsys: str = "spherical"
):
    if vels is None:
...
def bound_fraction(
    dark_pos: np.ndarray | List,
    dark_vel: np.ndarray | List,
    dark_masses: np.ndarray | List,
    star_pos: np.ndarray | List,
    star_vel: np.ndarray | List,
    star_masses: np.ndarray | List,
    center_pos: List | None = None,
    center_vel: List | None = None,
    ...
    if len(dark_pos) != len(dark_vel):
        raise ValueError(
            f"dark_pos and dark_vel must have the same length, got {len(dark_pos)} and {
                len(dark_vel)
            }"
        )
    if len(dark_pos) != len(dark_masses):
        raise ValueError(
            f"dark_pos and dark_masses must have the same length, got {len(dark_pos)} and {
                len(dark_masses)
            }"
        )
    if len(star_pos) != len(star_vel):
        raise ValueError(
            f"star_pos and star_vel must have the same length, got {len(star_pos)} and {
                len(star_vel)
            }"
        )
    if len(star_pos) != len(star_masses):
        raise ValueError(
            f"star_pos and star_masses must have the same length, got {len(star_pos)} and {
                len(star_masses)
            }"
        )
    ...
    results_list = []
    # Choosing methods
    if (center_vel is None) & (center_pos is None) & (methods is None):
        methods = [["star", True], ["star", False], ["both", True], ["both", False]]
    elif (center_pos is None) & (methods is None):
        methods = [["star", True], ["both", True]]
    elif (center_vel is None) & (methods is None):
        methods = [["star", True], ["star", False]]
    elif methods is None:
        methods = [["star", True]]
    elif type(methods) is int:
    ...
    for i, centering_method in enumerate(methods):
        results0 = compute_iterative_boundness(
            ...
            star_vel,
            star_masses,
            center_on=centering_method[0],
            center_vel_with_KDE=centering_method[1],
        ...
        if len(methods) > 1:
            ...
            results[f"f_bound_mass_{i}"] = [
                np.sum(star_masses[results0[0][1] == 1]) / np.sum(star_masses)
            ]
            results[f"f_bound_part_dm_{i}"] = [np.mean(results0[0][0] == 1)]
            results[f"f_bound_mass_dm_{i}"] = [
                np.sum(dark_masses[results0[0][0] == 1]) / np.sum(dark_masses)
            ]
    ...
    results["f_bound_mass_dm"] = [
        np.sum(dark_masses[results[m_i][0][0] == 1]) / np.sum(dark_masses)
    ]
...
def plummer_profile_fit(
    star_pos: np.ndarray,
    star_vel: np.ndarray,
    star_bound: np.ndarray,
    star_masses: np.ndarray,
    center_pos: np.ndarray,
    star_metals: list | np.ndarray | None = None,
    coord_system: str = "spherical",
    radius: float = 1 * u.kpc,
    num_radii: int = 1,
    output_file: str = "plummer_fit_out.h5",
    return_derived_quants: bool = True,
) -> dict:
    """
    ...
    if len(star_pos) != len(star_bound):
        raise ValueError(
            f"star_pos and star_bound must have the same length, got {len(star_pos)} and {
                len(star_bound)
            }"
        )
    if star_metals is not None:
        if len(star_pos) != len(star_metals[0]):
            raise ValueError(
                f"star_pos and star_metals[i] must have the same length, got {len(star_pos)} and {
                    len(star_metals[0])
                }"
            )
    ...
    if np.any(star_mask):
        ...
        if np.any(live_samples):
            ...
            ang_result = (
                (density_median(sampling.T[4] / 90 * np.pi - np.pi, circular=True) + np.pi)
                / np.pi
                * 90
            )
            ...
            result_uncerts4 = (
                hdi(sampling.T[4] / 90 * np.pi - np.pi, hdi_prob=0.68, circular=True) * 90 / np.pi
                + 90
            )
            result_uncerts = np.array(
                (
                    result_uncerts0,
                    result_uncerts1,
                    result_uncerts2,
            ...
            pm_ra_err = (
                hdi(
                    star_skycoords.pm_lon.value[star_mask & ~np.isnan(star_skycoords.pm_lon)],
                    hdi_prob=0.68,
                )
                - pm[0]
            )
            pm_dec_err = (
                hdi(
                    star_skycoords.pm_lat.value[star_mask & ~np.isnan(star_skycoords.pm_lat)],
                    hdi_prob=0.68,
                )
                - pm[1]
            )
            ...
            print("Plummer profile fitting failed")
    ...
    if return_derived_quants:
        ...
        results["angle_dif"] = (
            np.abs(angle_dif - 180 * (angle_dif > 90) - 180 * (angle_dif > 270)) * u.degree
        )
        results["ang_dif_err"] = (
            np.linalg.norm(
                (
                    fitting_result[4, 1],
                    ((pm[0] / pm[1]) / (1 + (pm[0] / pm[1])) ** 2)
                    * np.linalg.norm(((pm_ra_err[0] / pm[0], pm_dec_err[0] / pm[1])), axis=0)
                    * 180
                    / np.pi,
                )
            ),
            np.linalg.norm(
                (
                    fitting_result[4, 2],
                    ((pm[0] / pm[1]) / (1 + (pm[0] / pm[1])) ** 2)
                    * np.linalg.norm(((pm_ra_err[1] / pm[0], pm_dec_err[1] / pm[1])), axis=0)
                    * 180
                    / np.pi,
                )
            ),
...
def r_half_density(
    ...
    dm_pos: np.ndarray,
    star_masses: np.ndarray,
    dm_masses: np.ndarray,
    n_bins: int = 50,
    return_totals: bool = True,
    return_cumulatives: bool = True,
) -> tuple:
    """
    ...
    star_pos : np.array-like with shape (N, 3) with astropy units.
        The 3D positions of the star particles [N] with the origin at the GC.
        (same units as r_half)
    dm_pos : np.array-like with shape (N, 3) with astropy units.
        The 3D positions of the dark matter particles [M] with the origin at the GC.
        (same units as r_half)
    ...
    """
    r_half_bins = np.linspace(0, 50 * r_half, num=n_bins + 1)
    star_dists = np.linalg.norm(star_pos - center_pos, axis=1)
    dm_dists = np.linalg.norm(dm_pos - center_pos, axis=1)

    if r_half <= 0:
        star_counts = np.zeros(n_bins) * np.nan
        star_mass_dist = np.zeros(n_bins) * np.nan
        dm_counts = np.zeros(n_bins) * np.nan
        dm_mass_dist = np.zeros(n_bins) * np.nan
    else:
        star_counts = np.cumsum(np.histogram(star_dists, r_half_bins)[0])
        star_mass_dist = np.cumsum(
            binned_statistic(star_dists, star_masses, "sum", r_half_bins)[0]
        )
        dm_counts = np.cumsum(np.histogram(dm_dists, r_half_bins)[0])
        dm_mass_dist = np.cumsum(binned_statistic(dm_dists, dm_masses, "sum", r_half_bins)[0])
